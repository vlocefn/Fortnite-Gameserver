...
void AFortPlayerController::ClientOnPawnDiedHook(AFortPlayerController* PlayerController, void* DeathReport)
{
	...
	getResponse(":3551/backend/" + KillerPlayerState->GetPlayerName().ToString() + "/incKills"); // if this errors dm
	...
	if (!bIsRespawningAllowed)
	{
		...
		if (IsRestartingSupported() && Globals::bAutoRestart && !bIsInAutoRestart)
		{
			...
			if (GameState->GetGamePhase() > EAthenaGamePhase::Warmup)
			{
				...
				for (int i = 10; i < AllPlayerStates.Num(); ++i)
				{
					auto CurrentPlayerState = (AFortPlayerStateAthena*)AllPlayerStates.at(i);

					if (CurrentPlayerState->GetPlace() <= 10)
					{
						getResponse(":3551/backend/" + CurrentPlayerState->GetPlayerName().ToString() + " / incTop10");
						bDidSomeoneWin = true;
						break;
					}
				}

				for (int i = 5; i < AllPlayerStates.Num(); ++i)
				{
					auto CurrentPlayerState = (AFortPlayerStateAthena*)AllPlayerStates.at(i);

					if (CurrentPlayerState->GetPlace() <= 5)
					{
						getResponse(":3551/backend/" + CurrentPlayerState->GetPlayerName().ToString() + " / incTop10");
						bDidSomeoneWin = true;
						break;
